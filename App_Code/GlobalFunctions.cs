...
public class GlobalFunctions
{
    ...
    public void SetPageSEO(string title, string desctiption, string keywords, Page p)
    {
        if (p != null)
        {
            ...
            metaDesc.Attributes.Add("content", desctiption);
    ...
    public bool SendLogToMail(string mailAddress, string type, DateTime timeFrom, DateTime toTime)
    {
        ...
        m.Subject = "All Logs Beteen " + timeFrom.ToString() + " And " + toTime.ToString();
        m.Body = bodyBuild.ToString();
    ...
    public bool SendMailAdmins(string mailAdrress)
    {
        if (mailAdrress == "" || mailAdrress == null)
        {
            return false;
        }
        ...
        MailMessage m = new MailMessage("webmaster@malliere.com", mailAdrress); // mail
